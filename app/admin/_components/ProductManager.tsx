  ...
  return (
    <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col h-[calc(100vh-160px)]">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4 shrink-0">
        ...
        <button onClick={openAddModal} className="w-full sm:w-auto bg-blue-600 text-white px-6 py-3 rounded-xl font-bold hover:bg-blue-700 shadow-md active:scale-95 transition-all shrink-0">+ เพิ่มเมนูใหม่</button>
      </div>
        ...
        {filteredProducts.length === 0 ? <div className="text-center py-20 text-gray-400 font-medium border-2 border-dashed rounded-xl">ไม่พบเมนูที่ค้นหาครับ</div> : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5 pb-8">
            {filteredProducts.map((p: any) => {
              ...
              return (
                <div key={p.id} className={`p-5 border-2 rounded-2xl flex flex-col gap-4 transition-all duration-300 ${!isActive ? 'bg-gray-50 border-gray-200 opacity-80 grayscale-[30%]' : 'bg-white border-blue-50 hover:shadow-md hover:border-blue-100'}`}>
                  ...
                  <div className="grid grid-cols-2 gap-2 mt-auto pt-2">
                    <button onClick={() => openEditModal(p)} className="bg-yellow-50 text-yellow-700 hover:bg-yellow-100 active:scale-95 transition-all py-3 rounded-xl font-bold">✏️ แก้ไข</button>
                    
                    {isActive ? (
                      <button onClick={() => toggleActive(p)} className="bg-red-50 text-red-600 hover:bg-red-100 active:scale-95 transition-all py-3 rounded-xl font-bold">🗑️ ปิดการขาย</button>
                    ) : (
                      <button onClick={() => toggleActive(p)} className="bg-green-600 text-white hover:bg-green-700 active:scale-95 transition-all py-3 rounded-xl font-bold shadow-sm">✅ เปิดขายอีกครั้ง</button>
                    )}
                    
                    {isActive && (
                      <button onClick={() => toggleAvailable(p)} className={`col-span-2 py-3 rounded-xl font-bold border-2 active:scale-[0.98] transition-all ${available ? 'border-orange-200 text-orange-600 hover:bg-orange-50 hover:border-orange-300' : 'border-blue-200 text-blue-600 hover:bg-blue-50 hover:border-blue-300'}`}>
          ...
          <div className="bg-white rounded-3xl p-8 w-full max-w-lg shadow-2xl border border-gray-100 transform transition-all" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleSaveProduct} className="flex flex-col gap-4">
              ...
              <div className="flex gap-4 mt-6 pt-4 border-t border-gray-100">
                ...
                <button type="submit" className="flex-1 bg-blue-600 text-white hover:bg-blue-700 active:scale-95 transition-all py-4 rounded-xl font-bold shadow-md">💾 บันทึก</button>
              </div>
            </form>
          </div>
        </div>
